...
public class DisjointSets {
    ...
    public int union(int i, int j) {  
        /* TO DO Fill this method (The statement return 0 is here only to compile) */
    	// NOTE: i smaller, j larger rank
    	// if i and j in same set, should get same representative -> return rep.
    	if (find(i) == find(j))
    	{
    		return find(i);
    	}
    	...
    	{
    		// if 2 sets have same rank, set rank one larger
    		rank[find(j)]++;
    		// if ranks identical, merge i into j
    		par[find(i)] = find(j);
    		return find(j);
    	}
    	...
    	{
    		// dont update rank -> resulting rank is larger of two ranks
    		par[find(i)] = find(j);
    		return find(j);
    	}
